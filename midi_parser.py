...
def get_variable_length_number(tmpstr):
    sum = 0
    i = 0
    while 1:
        x = tmpstr[i]
...
class MidiEvent:
    ...
    def read(self, time, tmpstr):
        ...
        if channelVoiceMessages.has_value(y):
            ...
            length, tmpstr = get_variable_length_number(tmpstr[1:])
            ...
            length, tmpstr = get_variable_length_number(tmpstr[2:])
            self.data = tmpstr[:length]
            return tmpstr[length:]
        raise Exception("Unknown midi event type")
    ...
    def write(self):
        ...
        if channelVoiceMessages.hasattr(self.type):
            ...
            tmpstr = tmpstr + put_variable_length_number(len(self.data))
...
class DeltaTime(MidiEvent):
    ...
    def read(self, oldtmpstr):
        self.time, newtmpstr = get_variable_length_number(oldtmpstr)
        return self.time, newtmpstr

    def write(self):
        tmpstr = put_variable_length_number(self.time)
        return tmpstr
...
class MidiTrack:
    ...
    def read(self, tmpstr):
        ...
        length, tmpstr = get_number(tmpstr[4:], 4)
        self.length = length
    ...
    def write(self):
        ...
        return "MTrk" + put_number(len(tmpstr), 4) + tmpstr
...
class MidiFile:
    ...
    def readstr(self, tmpstr):
        ...
        length, tmpstr = get_number(tmpstr[4:], 4)
        assert length == 6
        format, tmpstr = get_number(tmpstr, 2)
        self.format = format
        assert format == 0 or format == 1  # dunno how to handle 2
        numTracks, tmpstr = get_number(tmpstr, 2)
        division, tmpstr = get_number(tmpstr, 2)
        if division & 0x8000:
    ...
    def writestr(self):
        ...
        tmpstr = "MThd" + put_number(6, 4) + put_number(self.format, 2)
        tmpstr = tmpstr + put_number(len(self.tracks), 2)
        tmpstr = tmpstr + put_number(division, 2)
